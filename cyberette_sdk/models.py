...
class Segment(BaseModel):
    ...
    verdict: str = Field(..., description="Detection verdict (REAL, DEEPFAKE, SUSPICIOUS)")


class Detection(BaseModel):
    """Core detection results."""
    verdict: str = Field(..., description="Overall verdict (REAL, DEEPFAKE, SUSPICIOUS)")
    percentage: Optional[float] = Field(None, description="Confidence percentage (0-100)")
    score: Optional[float] = Field(None, description="Detection score for images")
    segments: List[Segment] = Field(default_factory=list, description="Time-based segments")
